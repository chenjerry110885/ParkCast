...
**ARCHIVED 2026-09-21. Both tasks complete.** Task 1 shipped as `Q.ASSUMED_TS` (`6e43f50`); the
collector was rebuilt the same day, so the flag is meaningful from then on. Task 2 shipped as
`scripts/disk-growth.py` (`3ffd119`), and its first version undercounted the cold store by missing
`cold/meta/` — fixed in `8aeab63`. The measurement that closed the question: 65.4 MB/month against a
150–400 MB/month estimate, recorded in `docs/state-of-play.md` step 5.
